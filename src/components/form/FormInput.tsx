...
const FormInput = ({ label, placeholder, htmlFor, type, onChange, name, className, errors }: IFormInput) => {
    return (
        <div className="w-full flex flex-col gap-2 mt-7">
            ...
            <input id={htmlFor} type={type} name={name} className={`bg-transparent border border-signupTextColor rounded-md h-14 outline-none px-5 text-navTextDarkColor md:text-sm text-xs ${className}`} onChange={onChange} placeholder={placeholder} />
